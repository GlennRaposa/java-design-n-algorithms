...
public class Print2DArrayInSpiral {
	...
	public int printSpiral(int row_S, int row_E, int col_S, int col_E,
		...
		if(rowPrint == false)
		{
		   if(reverse == false){
			   for(int i=col_S; i<=col_E; i++){
				   System.out.println(arrA[row_S][i]);
			   }
		   }
		   row_S++;
		   rowPrint = true;
		...
		if(rowPrint == true){
			if(reverse == false){
				for(int i=row_S; i<=row_E; i++){
					 System.out.println(arrA[i][col_E]);
				}
			}
			col_E--;
			rowPrint = false;
		...
		if(rowPrint == false){
			if(reverse == true){
				for(int i=col_E; i>=col_S; i--){
					System.out.println(arrA[row_E][i]);
				}
			}
			row_E --;
			rowPrint = true;
		...
		if(rowPrint == true){
			if(reverse == true){
				for(int i=row_E; i>=row_S;i--){
					System.out.println(arrA[i][col_S]);
				}
			}
			col_S ++;
			rowPrint = false;
		...
		printSpiral(row_S, row_E, col_S, col_E,
				reverse, rowPrint);
